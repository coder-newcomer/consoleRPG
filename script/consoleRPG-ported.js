  ...
  profile: {
    ...
    clear: () => {
      ...
      if (localStorage.user == undefined && localStorage.currentuser == undefined) {
        return console.log(game.lang.parse(game.lang.profile.clearsuccess, currentlang));
      } else { console.warn(game.else); };
    },
    delete: (username, password) => experimental(),
    export: () => experimental(),
    import: (username, password, urlORjson) => experimental(),
    reset: () => experimental()
  }
};
