...
class XML { // multiple xml data support will be added soon
  ...
  */
  ...
  __document__ = {
    ...
    clear: () => { if (this.cleanMode) { this.__document__.dettach(); }; return this.cleanMode; },
    attach: () => {
      var currentid = 0;
      ...
      for (let i = 0; i < returned.length - 1; i++) { while (returned[i].id == currentid) { currentid += 1; }; };
      returned[returned.length - 1].id = this.id = currentid;
      this.__document__.style();
      this.__document__.element().innerHTML = this.value; // watchout
      return returned;
    },
    dettach: () => { this.__document__.element().remove(); },
    element: () => {
      var element;
      var returned = document.querySelector('html').querySelectorAll('xml');
      if (returned.length < 1) { this.__document__.attach(); };
      for (let i = 0; i < returned.length; i++) {
        if (returned[i].id == this.id) { element = returned[i]; }; // why this is sucks
      };
      return element;
    },
    nodeParse: (node) => {
